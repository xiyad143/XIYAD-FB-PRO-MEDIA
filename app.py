...
def exchange_long_lived_token(short_token):
    user = get_user()
    if not user.fb_app_id or not user.fb_app_secret: raise Exception('App credentials not set')
    url = "https://graph.facebook.com/v19.0/oauth/access_token"
    params = {'grant_type': 'fb_exchange_token', 'client_id': user.fb_app_id, 'client_secret': user.fb_app_secret, 'fb_exchange_token': short_token}
...
def fetch_facebook_user(token):
    resp = requests.get("https://graph.facebook.com/v19.0/me", params={'fields': 'name,picture{url}', 'access_token': token})
    data = resp.json()
    if 'error' in data: raise Exception(data['error']['message'])
    return {'id': data['id'], 'name': data['name'], 'picture': data.get('picture', {}).get('data', {}).get('url', '')}
...
# ---------- Routes (auth, pages, media, schedule, settings – same as before) ----------
# I'll include the new reset endpoint and keep the rest identical to previous version.
# (The full code is unchanged except for the added reset route.)
...
def connect_with_token():
    ...
    try:
        ...
        Page.query.delete()
        for p in pages:
            page = Page(id=p['id'], name=p['name'], access_token=p['access_token'], fan_count=p.get('fan_count',0),
                        picture_url=p.get('picture',{}).get('data',{}).get('url',''), category=p.get('category',''))
            db.session.add(page)
        db.session.commit()
...
def save_app_credentials():
    ...
    if not app_id or not app_secret: return jsonify({'error': 'Both fields required'}), 400
    user = get_user()
...
def facebook_oauth_start():
    ...
    fb_url = (f'https://www.facebook.com/v19.0/dialog/oauth?client_id={user.fb_app_id}&redirect_uri={redirect_uri}'
              f'&scope=pages_show_list,pages_read_engagement,pages_manage_posts,pages_manage_metadata&response_type=code')
    return redirect(fb_url)

@app.route('/connect/facebook/callback')
def facebook_oauth_callback():
    code = request.args.get('code')
    if not code: return 'Authorization failed', 400
    user = get_user()
    if not user.fb_app_id or not user.fb_app_secret: return 'App credentials missing', 400
    url = "https://graph.facebook.com/v19.0/oauth/access_token"
    params = {'client_id': user.fb_app_id, 'client_secret': user.fb_app_secret,
              'redirect_uri': request.host_url.rstrip('/') + '/connect/facebook/callback', 'code': code}
    resp = requests.get(url, params=params)
    data = resp.json()
    if 'error' in data: return f"Error: {data['error']['message']}", 400
...
def api_me():
    user = get_user()
    if not user.fb_long_lived_token: return jsonify({'authenticated': False})
    return jsonify({'authenticated': True, 'user': {'name': user.fb_user_name, 'picture': user.fb_user_picture}, 'pages_count': Page.query.count()})

@app.route('/api/pages')
def get_pages():
    pages = Page.query.all()
    return jsonify([{'id': p.id, 'name': p.name, 'fan_count': p.fan_count, 'picture_url': p.picture_url,
                     'category': p.category, 'new_followers': p.new_followers, 'reach': p.reach, 'engagement': p.engagement} for p in pages])
...
def page_schedule(page_id):
    ...
    if request.method == 'GET':
        if sched: return jsonify({'post_times': json.loads(sched.post_times) if sched.post_times else [], 'auto_publish': sched.auto_publish})
        ...
        if 'post_times' in data: sched.post_times = json.dumps(data['post_times'])
        if 'auto_publish' in data: sched.auto_publish = data['auto_publish']
        db.session.commit()
        return jsonify(success=True)
...
def upload_media():
    ...
    media = MediaFile(filename=filename, original_name=file.filename, file_size=os.path.getsize(filepath),
                      folder=folder, file_type=file_type, page_id=request.form.get('page_id'))
...
def get_scheduled():
    ...
    return jsonify([{'id': p.id, 'page_id': p.page_id, 'media_id': p.media_id, 'caption': p.caption,
                     'scheduled_time': p.scheduled_time.isoformat(), 'status': p.status, 'created_at': p.created_at.isoformat()} for p in posts])

@app.route('/api/settings', methods=['GET', 'POST'])
def settings():
    user = get_user()
    if request.method == 'GET':
        return jsonify({'fb_app_id': user.fb_app_id, 'fb_app_secret': user.fb_app_secret, 'groq_api_key': user.groq_api_key,
                        'timezone': user.timezone, 'developer_name': user.developer_name, 'developer_email': user.developer_email,
                        'developer_phone': user.developer_phone, 'developer_facebook': user.developer_facebook,
                        'developer_whatsapp': user.developer_whatsapp, 'developer_telegram': user.developer_telegram,
                        'developer_website': user.developer_website})
    else:
        data = request.json
        for field in ['fb_app_id','fb_app_secret','groq_api_key','timezone','developer_name','developer_email',
                      'developer_phone','developer_facebook','developer_whatsapp','developer_telegram','developer_website']:
            if field in data: setattr(user, field, data[field])
        db.session.commit()
        return jsonify({'success': True})

# ---------- NEW: Full Account Reset ----------
@app.route('/api/reset-account', methods=['POST'])
def reset_account():
    # Clear all user data
    user = get_user()
    # Delete all pages and related schedules
    PageSchedule.query.delete()
    ScheduledPost.query.delete()
    MediaFile.query.delete()
    Page.query.delete()
    # Reset user record to defaults but keep id=1
    user.fb_app_id = ''
    user.fb_app_secret = ''
    user.fb_long_lived_token = None
    user.fb_user_name = ''
    user.fb_user_picture = ''
    ...
    with scheduler.app.app_context():
        ...
        for sched in schedules:
            ...
            if current_time_str in times:
                ...
                if not media: media = MediaFile.query.filter_by(page_id=sched.page_id, file_type='photo').first()
                if not media: media = MediaFile.query.first()
                if media:
                    new_post = ScheduledPost(page_id=sched.page_id, media_id=media.id, caption=media.original_name,
                                            scheduled_time=now, status='pending')
                    db.session.add(new_post)
                    db.session.commit()
                    scheduler.add_job(id=f'post_{new_post.id}', func=execute_scheduled_post, args=[new_post.id],
                                      trigger='date', run_date=now, replace_existing=True)
...
with app.app_context():
    ...
    scheduler.add_job(id='auto_publisher', func=auto_publisher, trigger='interval', minutes=1, replace_existing=True)
